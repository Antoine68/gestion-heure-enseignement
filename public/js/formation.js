(function(){
    ...
    function listGlobalsElements(idPeriod){
        ...
        fetch(document.location.origin+'/formations/'+idFormation+'/periodes/'+ idPeriod +'/saisies-globales', {
            ...
            if (response.ok) {
                ...
                showNotification("fail", "Erreur: les éléments globaux n'ont pas pu être chargé");
            }
        });
    }
    ...
    function removeUsedSpeaker(element, idSpeaker){
        fetch(document.location.origin+'/elements/'+ element._id +'/retirer-intervenant', {
            ...
            if (response.ok) {
                ...
                showNotification("fail", "Erreur: l'intervenant n'as pas pu être retiré");
            }
        });
    }
    ...
    function addUsedSpeaker(element, idSpeaker){
        fetch(document.location.origin+'/elements/'+ element._id +'/ajouter-intervenant', {
            ...
            if (response.ok) {
                ...
                showNotification("fail", "Erreur: l'intervenant n'as pas pu être ajouté");
            }
        });
    }
        ...
        fetch(document.location.origin+'/elements/hebdomadaire/'+ idElement +'/modifier', {
            ...
            if (response.ok) {
                ...
                response.json().then(r => {
                    let errors = r.errors.errors;
                    message = "<ul>"
                    for(let i=0; i<errors.length; i++) {
                        message += "<li>" + errors[i].msg + "</li>";
                    }                   
                    message+= "</ul>"
                    showNotification("fail", "L'élément n'a pas pu être modifié: " + message, false);
                })
            }
        });
    }
    ...
    function editGlobalElementByPeriod(idPeriod, data){
        ...
        fetch(document.location.origin+'/periodes/'+ idPeriod +'/elements/global/modifier', {
            ...
            if (response.ok) {
                ...
                showNotification("success", "Eléments globaux modifiés");
            } else {
                response.json().then(r => {
                    let errors = r.errors.errors;
                    message = "<ul>"
    ...
    function formateGlobalForm(period) {
        ...
        editGlobalElementByPeriod(period._id, {subjects: data});
    }
    ...
    function manageModification(tdEditable){
        tdEditable.addEventListener("blur", function(){
            ...
            if( (!isNaN(tdEditable.textContent)) && (parseFloat(tdEditable.textContent) >= 0 || tdEditable.textContent === "")){
    ...
    function openModal(element){
        ...
        let text = {
            title: element.title || element.buildingElement.title || "non défini",
            nickname: element.nickname || element.buildingElement.nickname || "non défini",
            reference: element.reference || element.buildingElement.reference || "non défini",
            order: element.order || "non défini",
            input_type: element.input_type || "non défini",
        }
        content.innerHTML = "<p>Titre: "+ text.title +"</p><p>Surnom: "+ text.nickname +"</p><p>Reference: "+ text.reference +"</p><p>Ordre: "+ text.order + "</p><p>Type de saisie: " + text.input_type + "</b>";
        if (element.__t === "PedagogicalPeriod") {
            text.week = element.week;
            content.innerHTML += "<p>Nombre de semaines: " + text.week + "</p>";
        }
        if (typeof element.number_groups !== "undefined") {
        ...
        if (element.input_type !== "aucun") {
            content.innerHTML += "<label>Types de cours : <label>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>TP</span><span class='tag is-primary'> "+ (element.courses_types.TP ? "<i class='fas fa-check'></i>" : "<i class='fas fa-times'></i>") +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>TD</span><span class='tag is-primary'> "+ (element.courses_types.TD ? "<i class='fas fa-check'></i>" : "<i class='fas fa-times'></i>") +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>CM</span><span class='tag is-primary'> "+ (element.courses_types.CM ? "<i class='fas fa-check'></i>" : "<i class='fas fa-times'></i>") +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>PARTIEL</span><span class='tag is-primary'> "+ (element.courses_types.PARTIEL ? "<i class='fas fa-check'></i>" : "<i class='fas fa-times'></i>") +"</span></div></div>";    
        }
        if(element.input_type === "global") {
            content.innerHTML += "<label>Forfait : <label>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>TP</span><span class='tag is-primary'> "+ element.forfait.TP +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>TD</span><span class='tag is-primary'> "+ element.forfait.TD  +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>CM</span><span class='tag is-primary'> "+ element.forfait.CM  +"</span></div></div>";
            content.innerHTML += "<div class='control'><div class='tags has-addons'><span class='tag is-dark'>PARTIEL</span><span class='tag is-primary'> "+ element.forfait.PARTIEL +"</span></div></div>";    
        }
